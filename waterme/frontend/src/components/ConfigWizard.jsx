...
const ConfigWizard = () => {
                        ...
                        <div key={room.id} className={`bg-slate-900/40 border-2 ${room.enabled ? 'border-slate-800' : 'border-red-900/30'} p-10 rounded-[4rem] group hover:bg-slate-900 transition-all duration-500 relative overflow-hidden backdrop-blur-2xl shadow-2xl`}>
                            ...
                            <div className="flex justify-between items-start mb-10 relative z-10">
                                ...
                                <div className="flex gap-4">
                                    <button onClick={() => setEditingRoom(room)} className="p-4 bg-slate-800 hover:bg-blue-600 text-white rounded-2xl transition-all shadow-xl active:scale-95 border border-slate-700 hover:border-blue-400"><Edit2 size={18} /></button>
                                    <button onClick={async (e) => { e.stopPropagation(); if (confirm('Purge room?')) { await api.deleteRoom(room.id); loadConfig(); } }} className="p-4 bg-slate-800 hover:bg-red-600 text-white rounded-2xl transition-all shadow-xl active:scale-95 border border-slate-700 hover:border-red-400"><Trash size={18} /></button>
                                </div>
                            </div>

                            <h3 className="text-4xl font-black text-white mb-4 uppercase italic tracking-tighter leading-none">{room.name}</h3>
    ...
    return (
        <div className="max-w-7xl mx-auto pb-40 px-6 animate-in fade-in slide-in-from-bottom-12 duration-700">
            <button onClick={() => setEditingRoom(null)} className="flex items-center gap-3 text-slate-600 hover:text-white mb-12 transition-all font-black uppercase tracking-[0.4em] text-[10px] italic">
                            ...
                            <div className="bg-slate-950/50 p-10 rounded-[4rem] border-2 border-slate-800/50 shadow-inner">
                                <div className="flex items-center justify-between mb-10 border-b border-slate-800/30 pb-8">
                                    ...
                                    <button
                                        ...
                                        className={`w-16 h-16 rounded-3xl transition-all flex items-center justify-center ${editingRoom.use_fixed_schedule ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-600'}`}
                                    >
                                        <Clock size={28} />
                                    </button>
                                </div>
                                ...
                                {editingRoom.use_fixed_schedule ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 animate-in zoom-in-95 duration-500">
                                        <div className="space-y-4">
                                            ...
                                            <input type="time" value={editingRoom.lights_on_time} onChange={e => setEditingRoom({ ...editingRoom, lights_on_time: e.target.value })} className="w-full bg-slate-950 p-6 rounded-[2rem] text-3xl text-white font-black text-center border-2 border-slate-800 focus:border-blue-500 outline-none transition-all shadow-xl" />
                                        </div>
                                        <div className="space-y-4">
                                            <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.3em] italic px-2">Cycle Stop (OFF)</span>
                                            <input type="time" value={editingRoom.lights_off_time} onChange={e => setEditingRoom({ ...editingRoom, lights_off_time: e.target.value })} className="w-full bg-slate-950 p-6 rounded-[2rem] text-3xl text-white font-black text-center border-2 border-slate-800 focus:border-blue-500 outline-none transition-all shadow-xl" />
                                        </div>
                                    </div>
                                ) : (
                                    ...
                                    <div className="xl:col-span-8 flex flex-col gap-12">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 bg-slate-950/80 p-12 rounded-[4rem] border-2 border-slate-800/80 shadow-inner">
                                            <div className="flex flex-col gap-4">
                                                ...
                                                <input type="number" step="0.1" value={zone.dripper_rate_lph} onChange={e => updateZone(idx, 'dripper_rate_lph', parseFloat(e.target.value) || 0)} className="bg-slate-900 border-2 border-slate-800 rounded-[2rem] p-6 text-3xl text-white font-black text-center focus:border-blue-500 transition-all outline-none" />
                                            </div>
                                            <div className="flex flex-col gap-4">
                                                <label className="text-[10px] font-black text-slate-500 tracking-[0.3em] uppercase italic px-2">Units / Zone</label>
                                                <input type="number" value={zone.drippers_per_zone} onChange={e => updateZone(idx, 'drippers_per_zone', parseInt(e.target.value) || 0)} className="bg-slate-900 border-2 border-slate-800 rounded-[2rem] p-6 text-3xl text-white font-black text-center focus:border-blue-500 transition-all outline-none" />
                                            </div>
                                            <div className="flex flex-col gap-4">
                                                <label className="text-[10px] font-black text-slate-500 tracking-[0.3em] uppercase italic px-2">Units / Plant</label>
                                                <input type="number" value={zone.drippers_per_plant} onChange={e => updateZone(idx, 'drippers_per_plant', parseInt(e.target.value) || 0)} className="bg-slate-900 border-2 border-slate-800 rounded-[2rem] p-6 text-3xl text-white font-black text-center focus:border-blue-500 transition-all outline-none" />
                                            </div>
                                        </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-8 relative z-10">
                                                    <div className="flex flex-col gap-4">
                                                        ...
                                                        <div className="relative">
                                                            <input type="number" value={zone.p1_shots} onChange={e => updateZone(idx, 'p1_shots', parseInt(e.target.value) || 0)} className="w-full bg-slate-950 border-2 border-slate-800 rounded-2xl p-5 text-2xl text-white font-black text-center focus:border-blue-500 outline-none transition-all" />
                                            ...
                                            <div className="bg-orange-600/5 p-10 rounded-[4rem] border-2 border-orange-500/10 space-y-10 relative overflow-hidden group/p2">
                                                ...
                                                <div className="grid grid-cols-2 gap-8 relative z-10">
                                                    <div className="flex flex-col gap-4">
                                                        <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest italic px-2">Targets</span>
                                                        <div className="relative">
                                                            <input type="number" value={zone.p2_shots} onChange={e => updateZone(idx, 'p2_shots', parseInt(e.target.value) || 0)} className="w-full bg-slate-950 border-2 border-slate-800 rounded-2xl p-5 text-2xl text-white font-black text-center focus:border-orange-500 outline-none transition-all" />
                                                            <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-800 uppercase">Shots</div>
                                                        </div>
                                                    </div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-12 pt-12 border-t-2 border-slate-800/30">
                                            <div className="flex flex-col gap-4">
                                                ...
                                                <div className="relative">
                                                    <input type="number" value={zone.valve_delay_ms} onChange={e => updateZone(idx, 'valve_delay_ms', parseInt(e.target.value) || 0)} className="bg-slate-950 border-2 border-slate-800 rounded-[1.5rem] p-5 text-xl text-white font-black text-center w-40 focus:border-blue-500 outline-none transition-all" />
                                            ...
                                            <div className="flex flex-col gap-4">
                                                ...
                                                <div className="relative">
                                                    <input type="number" value={zone.stagger_minutes} onChange={e => updateZone(idx, 'stagger_minutes', parseInt(e.target.value) || 0)} className="bg-slate-950 border-2 border-slate-800 rounded-[1.5rem] p-5 text-xl text-white font-black text-center w-40 focus:border-blue-500 outline-none transition-all" />
